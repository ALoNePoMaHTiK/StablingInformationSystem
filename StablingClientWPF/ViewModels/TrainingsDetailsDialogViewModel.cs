...
namespace StablingClientWPF.ViewModels
{
    public class TrainingsDetailsDialogViewModel : BaseViewModel
    {
        ...
        private async Task CreateTrainingWithdrawing()
        {
            var result = await MaterialDesignThemes.Wpf.DialogHost.Show(
                new WithdrawingByTrainingDialog(
                    new WithdrawingByTrainingFormViewModel(CurrentDate, Training)), DAY_OPERATIONS_IDENTIFIER);
            if (result != null)
            {
        ...
        private async Task UpdateTrainingWithdrawing(Guid id)
        {
            var result = await MaterialDesignThemes.Wpf.DialogHost.Show(
                new WithdrawingByTrainingDialog(
                    new WithdrawingByTrainingFormViewModel(await _balanceWithdrawingsHttpClient.GetAsync(id),await _trainingsHttpClient.GetForShowAsync(Training.TrainingId))), DAY_OPERATIONS_IDENTIFIER);
            if (result != null)
            {
